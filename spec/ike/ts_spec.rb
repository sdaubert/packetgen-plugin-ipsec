      ...
      describe TrafficSelector do
        ...
        describe '#initialize' do
          ...
          it 'guesses type from addresses' do
            ...
            expect(ts.human_type).to eq('IPv4')
            expect(ts[:start_addr]).to be_a(PacketGen::Header::IP::Addr)
            expect(ts[:end_addr]).to be_a(PacketGen::Header::IP::Addr)
            ts = TrafficSelector.new(start_addr: '10::1', end_addr: '11::0')
            expect(ts.human_type).to eq('IPv6')
            expect(ts[:start_addr]).to be_a(PacketGen::Header::IPv6::Addr)
            expect(ts[:end_addr]).to be_a(PacketGen::Header::IPv6::Addr)
          end
        end
