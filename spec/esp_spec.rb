  ...
  module Plugin
    ...
    describe ESP do
      describe 'bindings' do
        it 'in IP packets' do
          expect(PacketGen::Header::IP).to know_header(ESP).with(protocol: 50)
        end

        it 'in IPv6 packets' do
          expect(PacketGen::Header::IPv6).to know_header(ESP).with(next: 50)
        end
        ...
        it 'in UDP packets' do
          ...
          int32.value = 0
          expect(PacketGen::Header::UDP).to_not know_header(ESP).with(dport: 4500, body: int32.to_s)
        end
      end

      describe '#initialize' do
        it 'creates a ESP Plugin with default values' do
          esp = ESP.new
      ...
      describe '#read' do
        ...
        it 'sets Plugin from a string' do
      ...
      context 'crypto' do
        ...
        describe '#encrypt!' do
          ...
          it 'encrypts a payload with CBC mode' do
            esp_pkt, red_pkt = get_packets_from('esp4-cbc.pcapng')
          ...
          it 'encryts a payload with CTR mode and authenticates it with HMAC-SHA256' do
            esp_pkt, red_pkt = get_packets_from('esp4-ctr-hmac.pcapng', icv_length: 12)
          ...
          it 'encrypts and authenticates a payload with GCM mode' do
            esp_pkt, red_pkt = get_packets_from('esp4-gcm.pcapng', icv_length: 16)
          ...
          it 'encrypts a payload with TFC' do
            esp_pkt, red_pkt = get_packets_from('esp-tfc.pcapng', icv_length: 16)
          ...
          it 'encrypts a payload with given padding' do
            red_pkt = read_packets('ipv6_tcp.pcapng').first
        ...
        describe '#decrypt!' do
          it 'decrypts a payload with CBC mode' do
            pkt, red_pkt = get_packets_from('esp4-cbc.pcapng')
          ...
          it 'decryts a payload with CTR mode and authenticates it with HMAC-SHA256' do
            pkt, red_pkt = get_packets_from('esp4-ctr-hmac.pcapng', icv_length: 12)
          ...
          it 'decrypts and authenticates a payload with GCM mode' do
            pkt, red_pkt = get_packets_from('esp4-gcm.pcapng', icv_length: 16)
          ...
          it 'decrypts a payload with TFC' do
            pkt, red_pkt = get_packets_from('esp-tfc.pcapng', icv_length: 16)
          ...
          it 'decrypts a payload with Extended SN' do
            ...
            expect(pkt.esp.body).to be_a(PacketGen::Header::ICMP)
          end
